...
namespace MarkdownDeep
{
    internal class SpanFormatter : StringScanner
    {
        ...
        // NOTE Called by Block & TableSpec rendering
        ...
        // Used to build an Id for headers
            ...
            while (!eof)
            {
                ...
                switch (current)
                {
                    case '*':
                        ...
                        break;
                    case '_':
                        token = ProcessSpan(current, TokenType.underline);
                        break;

                    case '`':
                        token = ProcessSpan(current, TokenType.code_span);
                        break;
                    case '~':
                        token = ProcessSpan(current, TokenType.strike);
                        break;
                    case '[':
                    case '!':
                        {
        ...
        // Process a ``` code ```
        Token ProcessSpan(char delim, TokenType tokenType)
        {
            int start = position;

            // Count leading ticks
            int tickcount = 0;
            while (SkipChar(delim))
            {
                tickcount++;
            }
            object data = null;
            if (tickcount == 3 && tokenType == TokenType.code_span)
            {
                // Try and parse the language specification
                data = ReadId();
            }
            ...
            //	CreateToken (tokenType, startofstrike, position - startofstrike,);

            // Create the token, move back to the end and we're done
            var ret = CreateToken(tokenType, startofstrike, position - startofstrike, data);
            position = endpos;
            return ret;
        }
        ...
        internal Token CreateToken(TokenType type, int startOffset, int length, object data = null)
        {
            if (m_SpareTokens.Count != 0)
            {
                ...
                return new Token(type, startOffset, length, data);
        }
        ...
        // FIXME you serious? is it really faster using this ?!?
        ...
        public IEnumerable<V> RenderToAny<T, V, S> (
            IMarkdownRenderer<T, V, S> renderer, 
            string str, 
            int start, 
            int len,
            List<S> startOfLine = null,
            TextStyle globalStyle = TextStyle.Normal)
            ...
            where S : ISpan
        {
            ...
            LinkInfo li;
            List<S> spans = startOfLine ?? new List<S>();
            //     if (l == null) l = renderer.NewLine(new List<S> { span } );
            foreach (Token t in m_Tokens)
            {
                switch (t.type)
                {
                    case TokenType.Text:
                        var span = renderer.Text(str, t.startOffset, t.length);
                        spans.Add(span);
                        break;
                    ...
                    case TokenType.img:
                        ...
                        {
                            var link_text = li.link_text.Substring(6).Trim();
                            if (li.link_text.StartsWith("audio:", StringComparison.Ordinal))
                                result.Add(renderer.Audio(li.def.url, link_text, li.def.title));
                            else if (li.link_text.StartsWith("video:"))
                                result.Add(renderer.Video(li.def.url, link_text, li.def.title));
                            else if (li.link_text.StartsWith("image:"))
                                result.Add(renderer.Image(li.def.url, link_text, li.def.title));
                            else
                                result.Add(renderer.Image(li.def.url, link_text, li.def.title));
                        }
                        else
                            result.Add(renderer.Image(li.def.url, li.link_text, li.def.title));
            ...
            if (spans.Count > 0)
            {
                var nline = renderer.NewLine(spans);
                result.Add(nline);
            }
            return result;
        }
