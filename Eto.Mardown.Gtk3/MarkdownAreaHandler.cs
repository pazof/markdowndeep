...
{
    public enum MarkdownAreaFormat  {
        Markdown = 3
    }

    // TODO create a Markdown property
    /// <summary>
    ...
    public class MarkdownAreaAreaHandler : TextAreaHandler<Gtk.TextView, MarkdownArea, MarkdownArea.ICallback>, MarkdownArea.IMarkdownArea, Markdown.ITextBuffer
    {
		...
		{
            ApplyTag(FamilyTagPrefix, typeface.Family.Name, range, tag =>
			{
                tag.Family = typeface.Family.Name;
				tag.FamilySet = true;
			});
		}
		...
		public FontTypeface SelectionTypeface
		{
			...
			set
			{
				if (value == null)
					return;
                var pangoFamily = value.Family.ToPango().Name;
                var pangoStyle = value.FontStyle == FontStyle.Italic ? Pango.Style.Italic : Pango.Style.Normal;
                var weight = value.Bold ? Pango.Weight.Bold : Pango.Weight.Normal;

                ApplySelectionTag(FamilyTagPrefix, value.Family.Name, tag =>
				{
                    tag.Family = pangoFamily;
					tag.FamilySet = true;
				});
                ApplySelectionTag(StyleTagPrefix, value.Style, tag =>
				{
                    tag.Style = pangoStyle;
					tag.StyleSet = true;
				});
                ApplySelectionTag(WeightTagPrefix, weight.ToString(), tag =>
				{
        ...
        public IEnumerable<int> SupportedFormats
		{
			get
			{
				yield return (int) RichTextAreaFormat.PlainText;
			}
		}

		public void Load(System.IO.Stream stream, int format)
		{
			switch (format)
			{
                case (int) RichTextAreaFormat.PlainText:
		...
		public void Save(Stream stream, int format)
		{
			switch (format)
			{
				case (int) RichTextAreaFormat.PlainText:
		...
		public Markdown.ITextBuffer Buffer
		{
			get { return this; }
		}
